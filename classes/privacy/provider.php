...
/**
 * Defines {@link \block_quizoverride\privacy\provider} class.
 *
...
namespace block_quizoverride\privacy;
...
/**
 * Privacy API implementation for the Quiz override plugin.
 *
 * @copyright  2019 UC Regents
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    /**
     * Describe all the places where the Quiz override plugin stores some personal data.
     *
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('auth_test_sometable', [
           'username' => 'privacy:metadata:db:authtestsometable:username',
           'timecreated' => 'privacy:metadata:db:authtestsometable:timecreated',
           'timemodified' => 'privacy:metadata:db:authtestsometable:timemodified',
        ], 'privacy:metadata:db:authtestsometable');

        $collection->add_database_table('auth_test_another', [
           'email' => 'privacy:metadata:db:authtestanother:email',
        ], 'privacy:metadata:db:authtestanother');

        $collection->add_user_preference('preferencename', 'privacy:metadata:preference:preferencename');
        $collection->add_user_preference('auth_test_anotherpreference', 'privacy:metadata:preference:authtestanotherpreference');
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if ($preferencename !== null) {
            writer::export_user_preference('block_quizoverride', 'preferencename', $preferencename,
                get_string('privacy:metadata:preference:preferencename', 'block_quizoverride'));
        }
        $authtestanotherpreference = get_user_preferences('auth_test_anotherpreference', null, $userid);

        if ($authtestanotherpreference !== null) {
            writer::export_user_preference('block_quizoverride', 'auth_test_anotherpreference', $authtestanotherpreference,
                get_string('privacy:metadata:preference:authtestanotherpreference', 'block_quizoverride'));
        }
    }
}
